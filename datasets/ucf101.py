...
# from .tools import VideoClips
from .tools import read_video
# from utils import ensure_dir


# class CustomUCF101(VisionDataset):
#     """
...
#     ) -> None:
#         super(CustomUCF101, self).__init__(root)
#         if not 1 <= fold <= 3:
...
#             data = [x.replace('\\', '/') for x in data]  # 2022-12-12 modified for Windows
#             selected_files.update(data)
#         # 2022-12-12 modified for Windows
#         # indices = [i for i in range(len(video_list)) if video_list[i] in selected_files]
#         indices = [i for i in range(len(video_list)) if video_list[i].replace('\\', '/') in selected_files]
#         return indices
#
...
#         video, _, info, video_idx = self.video_clips.get_clip(self.available_indices[idx])
...
class CustomUCF101(Dataset):
    ...
    def __init__(self,
                 ...
                 transform=None,
                 sample_ratio: float = 1.0):
        super(CustomUCF101, self).__init__()
        if not 1 <= fold <= 3:
            raise ValueError("fold should be between 1 and 3, got {}".format(fold))
        self.root = root
        self.fold = fold
    ...
    def _select_fold(self, video_list: List[str], annotation_path: str, fold: int, train: bool) -> List[int]:
        ...
        with open(f, "r") as fid:
            ...
            data = [x.strip().split(" ")[0] for x in data]
            data = [os.path.join(self.root, x) for x in data]
            data = [x.replace('\\', '/') for x in data]  # modified for Windows
            selected_files.update(data)
...
class UCF101Wrapper(ClsWrapper):
    IN_CHANNELS = 3
    NUM_CLASSES = 101
    ...
    def __init__(self,
                 ...
                 test_transform=None):
        super(UCF101Wrapper, self).__init__(root, train_transform, test_transform)
        self.data_root = os.path.join(self.root, 'UCF-101')
        self.annotation_path = os.path.join(self.root, 'ucfTrainTestlist')
    ...
    def load_split_and_targets(self, split: str):
        ...
        print(f'Loading UCF101 (split: {split}, sample_ratio: {sample_ratio})...')
        split_set = CustomUCF101(
