...
class HMDB51SkeletonDataset(SkeletonDataset):
    """
    Skeletons of HMDB51 dataset.
    """
    def __init__(self,
                 file_path: str,
                 fold: int = 1,
        ...
        self.fold = fold
        super(HMDB51SkeletonDataset, self).__init__(file_path, train, transform, sample_ratio)

    def _get_split(self):
        assert self.fold in [1, 2, 3], 'HMDB51SkeletonDataset only supports fold within [1,2,3].'
        split = 'train' if self.train else 'test'
        return f'{split}{self.fold}'


class HMDB51SkeletonWrapper(ClsWrapper):
    IN_CHANNELS = 17
    NUM_CLASSES = 51
    ...
    def __init__(self,
        ...
        if os.path.isdir(root):
            root = os.path.join(root, 'hmdb51.pkl')
        super(HMDB51SkeletonWrapper, self).__init__(root, train_transform, test_transform)
        assert 1 <= fold <= 3
        self.file_path = root
        self.fold = fold
    ...
    def load_split_and_targets(self, split: str):
        ...
        print(f'Loading HMDB51SkeletonDataset (split: {split}, sample_ratio: {sample_ratio})...')
        split_set = HMDB51SkeletonDataset(
            self.file_path,
...
class HMDB51RGBSkeletonDataset(RGBSkeletonDataset):
    """
    RGB+Skeletons of HMDB51 dataset.
    ...
    def __init__(self,
                 ...
                 sample_ratio: float = 1.0):
        self.fold = fold
        super(HMDB51RGBSkeletonDataset, self).__init__(rgb_root, skeleton_path,
                                                       train, transform, sample_ratio)

    def _get_split(self):
        assert self.fold in [1, 2, 3], 'HMDB51RGBSkeletonDataset only supports fold within [1,2,3].'
        split = 'train' if self.train else 'test'
        return f'{split}{self.fold}'
...
class HMDB51RGBSkeletonWrapper(ClsWrapper):
    NUM_CLASSES = 51
    ...
    def __init__(self,
                 ...
                 train_sample_ratio: float = 1.0,
                 test_sample_ratio: float = 1.0):
        super(HMDB51RGBSkeletonWrapper, self).__init__('Not applicable', train_transform, test_transform)
    ...
    def load_split_and_targets(self, split: str):
        ...
        transform = self.train_transform if split == 'train' else self.test_transform
        sample_ratio = self.train_sample_ratio if split == 'train' else self.test_sample_ratio
        print(f'Loading HMDB51RGBSkeletonDataset (split: {split}, sample_ratio: {sample_ratio})...')
        split_set = HMDB51RGBSkeletonDataset(
