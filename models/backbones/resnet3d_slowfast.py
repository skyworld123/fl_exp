...
from mmcv.cnn import ConvModule, kaiming_init, ConvTranspose3d
...
class ResNet3dPathway(ResNet3d):
    ...
    def __init__(self,
                 *args,
                 ...
                 fusion_kernel=5,
                 **kwargs):
        self.lateral_stem = lateral_stem
        self.lateral_layers = lateral_layers
        self.lateral_norm = lateral_norm
        self.speed_ratio = speed_ratio
        self.channel_ratio_stem = channel_ratio_stem
        self.channel_ratio_layers = channel_ratio_layers
        self.slow = slow
        self.fusion_kernel = fusion_kernel

        super().__init__(*args, **kwargs)
        self.inplanes = self.base_channels
        assert len(lateral_layers) == self.num_stages
        assert len(channel_ratio_layers) == self.num_stages

        if self.lateral_stem:
            # Assume the number of channels of the slow pathway is larger
            # than that of the fast pathway, and channel_ratio >= 1
            in_channels = self.inplanes // self.channel_ratio_stem if self.slow else self.inplanes * self.channel_ratio
            ...
            if self.slow:
                ...
                self.conv1_lateral = ConvTranspose3d(
                    in_channels,
                    out_channels,
                    kernel_size=(speed_ratio, 1, 1),
        ...
        for i in range(len(self.stage_blocks)):
            cum_i = sum([1 for x in self.stage_blocks[:i] if x > 0])
            planes = self.base_channels * 2**cum_i
            self.inplanes = planes * self.block.expansion if self.stage_blocks[i] > 0 else planes

            if self.lateral_layers[i] and i != self.num_stages - 1:
                # no lateral connection needed in final stage
                lateral_name = f'layer{(i + 1)}_lateral'
                in_channels = self.inplanes // self.channel_ratio_layers[i] \
                    if self.slow else self.inplanes * self.channel_ratio_layers[i]
                out_channels = 2 * in_channels
                if self.slow:
                    setattr(
                        self, lateral_name,
    ...
    def make_res_layer(self,
        ...
        lateral = self.lateral_layers[i - 1] if i > 0 else self.lateral_stem
        if lateral:
            channel_ratio = self.channel_ratio_layers[i - 1] if i > 0 else self.channel_ratio_stem
            if self.slow:
...
class ResNet3dSlowFast(nn.Module):
    ...
    Args:
        ...
        double_mode (bool): Set to False to input RGB only. Set to true to
            input RGB+pose (support PoseC3D).
    ...
    def __init__(self,
        ...
        super().__init__()
        self.pretrained = pretrained
        self.double_mode = double_mode  # True: RGB+pose; False: RGB only
    ...
    def forward(self, x):
        ...
        Args:
            x (torch.Tensor | tuple[torch.Tensor]): The input data.

        Returns:
            tuple[torch.Tensor]: The feature of the input samples extracted
                by the backbone.
        """
        if not self.double_mode:  # single input, assume RGB only
            ...
            x_fast = nn.functional.interpolate(
                ...
                scale_factor=(1.0 / (self.resample_rate // self.speed_ratio), 1.0,
                              1.0))
        else:  # double input, RGB and pose
            assert isinstance(x, list) or isinstance(x, tuple) and len(x) == 2
            x_slow, x_fast = x
        ...
        if self.slow_path.lateral_stem:
            x_fast_lateral = self.slow_path.conv1_lateral(x_fast)
            x_slow = torch.cat((x_slow, x_fast_lateral), dim=1)
        if self.fast_path.lateral_stem:
            x_slow_lateral = self.fast_path.conv1_lateral(x_slow)
            x_fast = torch.cat((x_fast, x_slow_lateral), dim=1)

        for i in range(self.slow_path.num_stages):
            layer_name = f'layer{i + 1}'
            if hasattr(self.slow_path, layer_name):
                res_layer_slow = getattr(self.slow_path, layer_name)
                x_slow = res_layer_slow(x_slow)
                if i == 0 and self.slow_path.with_pool2:
                    x_slow = self.slow_path.pool2(x_slow)
            if hasattr(self.fast_path, layer_name):
            ...
            x_slow_bak, x_fast_bak = x_slow, x_fast
            lateral_name = f'layer{(i + 1)}_lateral'
            if i != len(self.slow_path.res_layers) - 1 and self.slow_path.lateral_layers[i]:
                conv_lateral = getattr(self.slow_path, lateral_name)
                x_fast_lateral = conv_lateral(x_fast_bak)
                x_slow = torch.cat((x_slow_bak, x_fast_lateral), dim=1)
            if i != len(self.fast_path.res_layers) - 1 and self.fast_path.lateral_layers[i]:
                conv_lateral = getattr(self.fast_path, lateral_name)
                x_slow_lateral = conv_lateral(x_slow_bak)
                x_fast = torch.cat((x_fast_bak, x_slow_lateral), dim=1)
