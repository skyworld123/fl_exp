...
def get_similarity(grad_1, grad_2, distance_type="L1"):
    if distance_type == "L1":
        ...
        raise NotImplementedError(f'Unknown distance type "{distance_type}"')


def get_gradients(global_model, local_models):
    """return the `representative gradient` formed by the difference between
    the local work and the sent global model"""
    def state_dict_np(state_dict) -> list:
        return [param.detach().cpu().numpy() for param in state_dict.values()]

    global_model_params = state_dict_np(global_model)
    local_model_params = []
    for model in local_models:
        if model is None:
            local_model_params.append(global_model_params)
        else:
            local_model_params.append(state_dict_np(model))
...
def get_matrix_similarity(global_model, local_models, distance_type):
    n_clients = len(local_models)
    local_model_grads = get_gradients(global_model, local_models)
    metric_matrix = np.zeros((n_clients, n_clients))
    for i, j in product(range(n_clients), range(n_clients)):
        metric_matrix[i, j] = get_similarity(
            local_model_grads[i], local_model_grads[j], distance_type
